...
function ProjectFrame({ className, theme, title, role, image }: Props) {
  ...
  return (
    <>  
            <div className={`p-0 flex flex-col border border-[6px] sm:border-[10px] rounded-[30px] w-[70vw] h-[30vh] lg:w-[50vw] sm:w-[90vw] md:w-[90vw] sm:h-[40vh] md:h-[40vh] lg:h-[50vh] border-[#ffffff7d] overflow-hidden ${className}`}>
                <div onMouseEnter={handleMouseEnter} onMouseLeave = {handleMouseLeave} className={`${isHovering?`transition-all ease-in-out duration-300 hover:bg-gradient-to-b from-[${theme}] to-[#1E1E1E] parent`:`transition-all ease-in-out duration-300 parent`}`}>                
                    <div className='project-header flex flex-row justify-between items-center'>
                        <div className='p-3 px-5'>
                            <div>
                                <GlowingText className='text-2xl md:text-3xl font-medium'>{title}</GlowingText>      
                            </div>
                            <div>
                    ...
                    <div className='child project-image flex overflow-hidden items-center justify-center relative top-[2rem] sm:top-[6rem] md:top-[6rem] lg:top-16 sm:px-10 px-5 lg:px-20 transition-all ease-in-out duration-300'>
